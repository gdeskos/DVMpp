#ifndef VORTEXSHEET_HPP
#define VORTEXSHEET_HPP
...
class VortexSheet
{
    ...
    /// Maximum number of vortices allowed in each panel
    ...
    void read_input_coord(const std::string& domain);
    ...
    [[nodiscard]] bool inside_body(double xcoor, double zcoor) const;

    /// Mirror the particle from a panel
    [[nodiscard]] Vector mirror(double x_init,
                                double z_init,
                                double x_0,
                                double z_0,
                                double x_1,
    ...
    [[nodiscard]] VortexBlobs release_nascent_vortices_rw(Random &_rand);
    ...
    void print_collocation() const;

    /// Print the unit vector for each collocation point
    void print_unit_vectors() const;

    /// Print the surface vorticity for each collocation point
    void print_gamma() const;
    ...
    [[nodiscard]] std::tuple<double, double> get_forces() const;
